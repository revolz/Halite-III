#!/usr/bin/env python3
"""
rl_v7 / bc_train.py  --  behavioral cloning: train the network to imitate rl_v5.

Loads features.csv + patches.npy, splits train/val, and minimises a
class-weighted cross-entropy between the network's action and rl_v5's action.
STAY dominates the data, so per-class inverse-frequency weights (capped) keep
the rarer moves from being ignored.

Reports overall and per-action match-rate vs rl_v5 on a held-out validation
split.  The phase-1 goal is val match-rate >= ~90%, at which point the BC bot
should deposit in rl_v5's ballpark (verify with rl_eval.py).

Usage:
    python rl_v7/bc_train.py --epochs 30
...
def main():
    ap = argparse.ArgumentParser(description='Behavioral cloning for rl_v7.')
