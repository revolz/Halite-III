...
class ActorCritic(nn.Module):
    ...
    @classmethod
    def load_expand(cls, path: str, device: str = 'cpu', **kwargs) -> 'ActorCritic':
        """Warm-start from a checkpoint whose actor head may be SMALLER than this
        model's (vocabulary expansion, e.g. 6→8 actions).  Copies every parameter
        whose shape matches; for the actor head, copies the overlapping leading
        action rows and leaves the new rows (HOME/PROSPECT) at their init.  All of
        the vision/trunk/critic transfers, so the policy keeps what it learned.
        """
        model  = cls(**kwargs)
        sd_new = model.state_dict()
        sd_old = torch.load(path, map_location=device)
        copied = partial = 0
        for k, v in sd_new.items():
            ...
            ov = sd_old[k]
            if ov.shape == v.shape:
                sd_new[k] = ov; copied += 1
            elif k in ('actor.weight', 'actor.bias') and ov.shape[0] <= v.shape[0]:
                v[:ov.shape[0]] = ov              # transfer overlapping action rows
                partial += 1
