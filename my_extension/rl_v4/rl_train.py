...
from rl_features import N_SHIP_ACTIONS, target_dropoffs, spawn_econ_ok
...
DEFAULTS = dict(
    ...
    clip_eps            = 0.2,
    vf_coef             = 0.5,
    ent_coef            = 0.20,    # strong: prevents the entropy→0 policy collapse
    ent_floor           = 0.5,     # nats — keep healthy exploration
    ent_floor_coef      = 0.5,     # strong floor penalty when entropy < ent_floor
    ...
    # Opponent regime:
    #   'curriculum' (default): idle → greedy → self-play
    ...
    def _collect_episode(self, env: HaliteEnv) -> Tuple[List[ShipTrajectory], float, List[int]]:
        ...
        while not done:
            ...
            for ship_id, (spatial, scalars, mask) in obs.items():
                ...
                action, log_prob, value = self.model.select_action(sp_t, sc_t, mask=mask)
