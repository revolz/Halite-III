...
def _should_spawn(game: hlt.Game, me) -> bool:
    """Reserve-aware spawn gate (shared economy logic) + factory-occupancy guard."""
    gmap       = game.game_map
    turns_left = game_max_turns(gmap.width, gmap.height) - game.turn_number
    n_ships    = len(list(me.get_ships()))
    tgt        = target_dropoffs(gmap.width, gmap.height)
    if not spawn_econ_ok(me.halite_amount, n_ships, turns_left,
                         len(list(me.get_dropoffs())), tgt):
        return False
    if gmap[me.shipyard].is_occupied:
        return False
...
def main(model_path: str, device_str: str = 'cpu', deterministic: bool = False):
    ...
    while True:
        ...
        # First pass: select a raw (mask-gated) action for every ship.
        raw: dict = {}
        for ship in ships:
            ...
            if deterministic:
                action_idx = model.greedy_action(sp_t, sc_t, mask=mask)
            else:
