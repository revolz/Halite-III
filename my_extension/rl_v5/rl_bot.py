...
from rl_features import (
    ...
    ACTION_RANDOM, ACTION_HOME, ACTION_PROSPECT, ACTION_DROPOFF,
    PROSPECT_RADIUS,
    dropoff_legal, build_action_mask, target_dropoffs, spawn_econ_ok,
    game_max_turns, ENDGAME_COLLAPSE_TURNS,
    compute_home_cost_field, least_cost_home_step,
    fsm_feature_vector, fsm_prior_bonus,
    FSMController, FSMWorld, FSM_PRIOR_LOGIT, ESCAPE,
)
...
def _fsm_world_hlt(game: hlt.Game, me) -> FSMWorld:
    """Build the turn-global FSMWorld from the hlt API (mirrors env._fsm_world)."""
    gmap = game.game_map
    W, H = gmap.width, gmap.height
    halite = {(x, y): gmap[Position(x, y)].halite_amount
              for x in range(W) for y in range(H)}
    deposits = [(me.shipyard.position.x, me.shipyard.position.y)] + \
               [(d.position.x, d.position.y) for d in me.get_dropoffs()]
    friendly_pos = {(s.position.x, s.position.y) for s in me.get_ships()}
    return FSMWorld(
        halite=halite,
        deposits=deposits,
        home_cost_field=compute_home_cost_field(halite, deposits, W, H),
        friendly_pos=friendly_pos,
        W=W, H=H,
        turns_left=game_max_turns(W, H) - game.turn_number,
    )
...
def extract_spatial_hlt(
    ...
    enemy_positions: list = []                 # for Ch 12
    friendly_cargo_map: dict = {}              # for Ch 13
...
def extract_scalars_hlt(
    ...
    gmap       = game.game_map
    W, H       = gmap.width, gmap.height
    # True game length for this map (engine advertises 500 to bots regardless of
    # map size; a 32×32 game ends at 400).  Using the true value keeps the
    # turns_remaining/endgame features consistent with training.
...
_home_field_cache = {'turn': -1, 'field': None}


def _home_cost_field_hlt(game: hlt.Game, me) -> dict:
    """Least-cost-to-deposit field (rl_v5), cached once per turn.  Mirrors
    HaliteEnv._home_cost_field so train and inference home identically."""
    turn = game.turn_number
    if _home_field_cache['turn'] != turn:
        gmap = game.game_map
        W, H = gmap.width, gmap.height
        halite = {(x, y): gmap[Position(x, y)].halite_amount
        ...
        deposits = ([(me.shipyard.position.x, me.shipyard.position.y)]
                    + [(d.position.x, d.position.y) for d in me.get_dropoffs()])
        _home_field_cache['turn'] = turn
        _home_field_cache['field'] = compute_home_cost_field(halite, deposits, W, H)
    return _home_field_cache['field']


def _home_dir_hlt(ship, game: hlt.Game, me) -> int:
    """Primitive action (0–4) toward the nearest deposit along the least-COST path
    (rl_v5; was pure Manhattan in rl_v4).  See least_cost_home_step."""
    gmap  = game.game_map
    field = _home_cost_field_hlt(game, me)
    return least_cost_home_step(ship.position.x, ship.position.y,
                                field, gmap.width, gmap.height)
...
HOME_CARGO_THRESHOLD = 0.90   # return home when cargo >= 90% of MAX_HALITE
                              ...
                              #  "fully loaded" trigger; keeps bot inference consistent
                              #  with training)
ENDGAME_BUFFER       = 5      # force home if turns_left <= dist_to_deposit + buffer


def action_mask_hlt(game: hlt.Game, ship, me) -> np.ndarray:
    """Build the bool[N_SHIP_ACTIONS] action mask for a ship from hlt state.

    Mirrors action_mask_from_engine so train and inference offer the policy the
    same set of legal actions (in particular, when DROPOFF is allowed)."""
    gmap   = game.game_map
    ...
    cell_owned = gmap[ship.position].has_structure
    ok = dropoff_legal(bank, cell_h, cargo, dist, num_dropoffs, turns_left, cell_owned)
    return build_action_mask(ok)


def _spawn_econ_ok(game: hlt.Game, me) -> bool:
    """Reserve-aware spawn economic gate (shared economy logic only).

    rl_v5: the factory-occupancy guard is no longer here — a spawn only lands on
    the factory cell, so the real constraint is just that no friendly ENDS on the
    factory this turn.  That is enforced in the turn loop after collision
    resolution (factory reservation + post-cascade free check), mirroring
    rl_env._build_commands.  rl_v4's blanket "no friendly adjacent" guard was far
    too conservative and stalled early fleet growth.
    """
    gmap       = game.game_map
    turns_left = game_max_turns(gmap.width, gmap.height) - game.turn_number
    n_ships    = len(list(me.get_ships()))
    tgt        = target_dropoffs(gmap.width, gmap.height)
    return spawn_econ_ok(me.halite_amount, n_ships, turns_left,
                         len(list(me.get_dropoffs())), tgt)
...
def main(model_path: str, device_str: str = 'cpu', deterministic: bool = False):
    ...
    while True:
        ...
        # Advance the per-ship FSM once this turn (same module env uses).
        world = _fsm_world_hlt(game, me)
        fsm.begin_turn([s.id for s in ships])

        # First pass: select a raw (mask-gated) action for every ship.  The FSM
        # suggestion is fed as features AND as a logit prior so the policy follows
        # the state machine by default and only overrides where it has learned to.
        raw: dict = {}
        for ship in ships:
            sx, sy = ship.position.x, ship.position.y
            cell_h = game.game_map[ship.position].halite_amount
            fsm_action, fsm_state = fsm.decide(ship.id, sx, sy,
                                               ship.halite_amount, cell_h, world)

            spatial = extract_spatial_hlt(game, ship.position, me)
            scalars = np.concatenate([extract_scalars_hlt(game, ship, me),
                                      fsm_feature_vector(fsm_state, fsm_action)])
            mask    = action_mask_hlt(game, ship, me)
            ...
            if deterministic:
                action_idx = model.greedy_action(sp_t, sc_t, mask=mask, prior_bonus=prior)
            else:
                action_idx, _, _ = model.select_action(sp_t, sc_t, mask=mask,
                                                       prior_bonus=prior)
            raw[ship.id] = (ship, action_idx, mask)

        # Determine the single dropoff ship this turn (first legal DROPOFF pick).
        dropoff_ship = None
        for ship in ships:
            _s, a, mask = raw[ship.id]
            if a == ACTION_DROPOFF and mask[ACTION_DROPOFF]:
                dropoff_ship = ship
                break
        dropoff_ship_ids = {dropoff_ship.id} if dropoff_ship else set()

        # Second pass: resolve movement for non-dropoff ships.
        ...
        for ship in ships:
            ...
            endgame    = ship.halite_amount > 0 and turns_left <= dist_dep + ENDGAME_BUFFER
            if endgame:
            ...
            elif fsm.state.get(ship.id) == ESCAPE:
                homing_ships.discard(ship.id)
            # Auto-trigger home if cargo exceeds threshold
            elif ship.halite_amount >= constants.MAX_HALITE * HOME_CARGO_THRESHOLD:
                homing_ships.add(ship.id)
        ...
        # Phase 3c — Reserve the factory cell for a spawn (rl_v5).  If the economy
        # gate says spawn this turn, force any friendly heading onto the factory to
        # STAY off it, BEFORE the Phase 4 cascade resolves induced conflicts.
        want_spawn = _spawn_econ_ok(game, me)
        fxy = me.shipyard.position
        if want_spawn:
            for ship, _ in ship_resolved:
                if ship.id in dropoff_ship_ids:
                    continue
                if dest_of[ship.id] == fxy and ship_pos[ship.id] != fxy:
                    overridden_act[ship.id] = ACTION_STAY
                    dest_of[ship.id] = ship_pos[ship.id]
        ...
        # heading onto one of our own deposit cells are exempt from the cascade — they
        # pile onto the shipyard/dropoff and the engine banks each ship's cargo before
        # the wreck (the "home sacrifice").  Enemy avoidance above still applies.
        deposit_cells = {me.shipyard.position}
        deposit_cells.update(d.position for d in me.get_dropoffs())
        endgame_collapse = (game_max_turns(W, H) - game.turn_number) < ENDGAME_COLLAPSE_TURNS
        committed = {ship.id for ship, _ in ship_resolved
                     if endgame_collapse and dest_of[ship.id] in deposit_cells}
        for _ in range(len(ship_resolved)):
            pos_occupants: dict = {}
            for ship, _ in ship_resolved:
                if ship.id in committed:
                    continue
        ...
        # Spawn iff the economy gate fired and the factory cell ends free this turn
        # (reservation above already yielded any friendly that targeted it).
        if want_spawn:
            factory_taken = any(dest_of[s.id] == fxy for s, _ in ship_resolved)
            if not factory_taken:
