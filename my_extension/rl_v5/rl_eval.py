...
from rl_features import target_dropoffs, spawn_econ_ok, fsm_prior_bonus, FSM_PRIOR_LOGIT
...
def run_game(
    ...
    while not done:
        ...
        for ship_id, (spatial, scalars, mask) in obs.items():
            ...
            if deterministic:
                action = model.greedy_action(sp_t, sc_t, mask=mask, prior_bonus=prior)
            else:
                action, _, _ = model.select_action(sp_t, sc_t, mask=mask, prior_bonus=prior)
...
_BOT_DIRS = {
    'rl_v4': os.path.join(_MY_EXT, 'rl_v4'),   # the champ rl_v5 must beat (default)
...
def main():
    ...
    parser.add_argument('--opponent',      default='rl_v4',        help='Opponent: greedy|idle|random (scripted) or rl_v4|rl_v3|rl_v2 (head-to-head). Default: rl_v4')
