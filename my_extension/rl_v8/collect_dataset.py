#!/usr/bin/env python3
"""
rl_v8 / collect_dataset.py  --  turn V71 (2019 bot) replays into a
behavioral-cloning dataset.

For every replay in the replays directory we walk the game turn by turn,
reconstruct the board state at the START of each turn, and for each V71 ship
emit one row:

    features (the ~45 named scalars)  +  a 9x9x6 local map patch  +  the action
    V71 actually took that turn (STAY / N / E / S / W / DROPOFF).

Outputs (in rl_v8/dataset/):
...
Only players whose recorded replay name is "RevolzBot" (V71's game.ready()
name) are collected, so self-play replays contribute both seats and
V71-vs-greedy replays contribute only the V71 seat -- automatically.
...
Usage:
    ...
    python rl_v8/collect_dataset.py --stay-keep 0.4     # subsample STAY rows
"""
...
def collect_replay(path, rows, patches, counts, align, rng, stay_keep, game_id):
    ...
    # which seats are V71 (reports itself as "RevolzBot")
    collect_pids = [p['player_id'] for p in replay['players']
                    if p.get('name') == 'RevolzBot']
    if not collect_pids:
        # fall back: if nothing is named RevolzBot, collect player 0
        collect_pids = [0]
...
def main():
    ap = argparse.ArgumentParser(description='Build the rl_v8 BC dataset from replays.')
