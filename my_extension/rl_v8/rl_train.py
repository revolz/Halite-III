#!/usr/bin/env python3
"""
rl_v8 / rl_train.py  --  PPO fine-tuning from a BC warm-start vs a frozen
opponent bot.

**Primary objective (default): beat V71**, the 2019 bot rl_v8 was imitating
via BC. rl_v5 and rl_v7 remain available as secondary/alternate opponents
(--opponent rl_v5 / rl_v7 / alternate / all) for regression-checking or
multi-opponent robustness, but the default training target is V71 itself --
i.e. rl_v8 should end up a *better* bot than the one it learned from.

Loads a BC checkpoint, then runs PPO against the chosen frozen opponent bot.
...
Key stability choices carried over from rl_v7's tuned defaults (unrelated to
the choice of opponent bot):
...
Usage:
    python rl_v8/rl_train.py --bc-ckpt checkpoints/bc.pt --episodes 500          # vs V71 (default)
    python rl_v8/rl_train.py --bc-ckpt checkpoints/bc.pt --opponent all --episodes 500
    python rl_v8/rl_train.py --resume checkpoints/ppo_ep100.pt --start-ep 101 --opponent rl_v5
"""
...
from rl_env import HaliteEnvV8, opponent_cmd_for    # noqa: E402
...
def collect_trajectories(env, policy_fn, n_games, seed_base, opponent_cmds=None):
    """opponent_cmds: optional list to round-robin the opponent per game
    (used by --opponent alternate so rl_v8 doesn't overfit to just one bot)."""
    trajs, infos = [], []
    for i in range(n_games):
        env.seed = seed_base + i
        if opponent_cmds:
            env.opponent_cmd = opponent_cmds[i % len(opponent_cmds)]
...
def main():
    ap = argparse.ArgumentParser(description='PPO fine-tune rl_v8 vs V71 (default), rl_v5, and/or rl_v7.')
    ...
    ap.add_argument('--opponent', choices=['v71', 'rl_v5', 'rl_v7', 'alternate', 'all'],
                    default='v71',
                    help="frozen opponent bot for PPO (default 'v71' -- the primary "
                         "objective is to beat the 2019 bot rl_v8 imitated; "
                         "'alternate' round-robins rl_v5/rl_v7; 'all' round-robins "
                         "v71+rl_v5+rl_v7)")
    ap.add_argument('--v5-model', default=None, help='override rl_v5 checkpoint path')
    ap.add_argument('--v7-model', default=None, help='override rl_v7 checkpoint path')
    ...
    v71_cmd = opponent_cmd_for('v71')
    v5_cmd  = opponent_cmd_for('rl_v5', args.v5_model)
    v7_cmd  = opponent_cmd_for('rl_v7', args.v7_model)
    if args.opponent == 'v71':
        opponent_cmds = [v71_cmd]
    elif args.opponent == 'rl_v5':
        opponent_cmds = [v5_cmd]
    elif args.opponent == 'rl_v7':
        opponent_cmds = [v7_cmd]
    elif args.opponent == 'alternate':
        opponent_cmds = [v5_cmd, v7_cmd]
    else:  # 'all'
        opponent_cmds = [v71_cmd, v5_cmd, v7_cmd]
    print(f"PPO opponent mode: {args.opponent}  ({len(opponent_cmds)} bot(s) round-robin)")

    env = HaliteEnvV8(width=args.width, height=args.height,
                      opponent_cmd=opponent_cmds[0])
    ...
    for ep in range(args.start_ep, args.start_ep + args.episodes):
        ...
        trajs, infos = collect_trajectories(env, policy_fn, args.games_per_update,
                                            seed_counter, opponent_cmds=opponent_cmds)
