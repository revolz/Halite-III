#!/usr/bin/env python3
"""
rl_v8 / rl_env.py  --  Gym-style single-player RL environment.

Wraps the Python Halite engine for PPO training.  Player 0 is the rl_v8
agent; player 1 is a frozen opponent (V71, rl_v5, or rl_v7) driven via
FrozenOpponent, which just launches an arbitrary bot subprocess and pipes
turn messages to it -- the opponent doesn't need to be one of our own bots,
it only needs to speak the standard hlt stdin/stdout protocol. V71 is the
primary PPO objective (rl_v8 must learn to beat the bot it was imitating);
rl_v5/rl_v7 remain available as secondary opponents.

The agent provides one action per live ship each turn.  The environment
handles spawning (rl_v5's economy rule, reused for parity) and collision
resolution (the deterministic resolver) internally, so the agent only ever
chooses strategy.
...
def opponent_cmd_for(name: str, model_path: Optional[str] = None) -> str:
    """Build a subprocess command string for a named opponent
    ('rl_v5', 'rl_v7', or 'v71' -- the 2019 imitation target, now also the
    PPO objective bot)."""
    if name == 'rl_v5':
        ...
        mp = model_path or RL_V7_DEFAULT_MODEL
        script = os.path.join(MY_EXT, 'rl_v7', 'rl_bot.py')
        return f'python -u "{script}" --model "{mp}" --deterministic'
    elif name == 'v71':
        # V71 has no in-process model / --deterministic flag -- it's its own
        # fixed subprocess, always the same script.
        return f'python -u "{V71_SCRIPT}"'
    else:
        raise ValueError(f"unknown opponent {name!r} (expected 'rl_v5', 'rl_v7', or 'v71')")


class FrozenOpponent:
    """Drive an arbitrary frozen bot in a subprocess (same protocol as run_game.py)."""
...
class HaliteEnvV8:
    """Wraps HaliteEngine for single-agent PPO (player 0 = rl_v8)."""

    def __init__(self, width=32, height=32, seed=None,
                 opponent_cmd=None, agent_pid=0):
        self.width  = width
        self.height = height
        self.seed   = seed
        self.agent_pid = agent_pid
        if opponent_cmd is None:
            opponent_cmd = opponent_cmd_for('v71')
        self.opponent_cmd = FrozenOpponent(opponent_cmd).get_cmd()
